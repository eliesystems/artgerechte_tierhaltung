<template>
	<div class="max-w-4xl mx-auto flex flex-col flex-grow">
		<div v-if="answerLoaded && currentQuestionnaireComponent" class="max-w-4xl mx-auto px-12 py-2 flex-grow">
			<component :is="currentQuestionnaireComponent" :wellbeing-store="wellbeingStore" />
		</div>
...
const wellbeingStore = useWellbeingStore();
...
const saveResources = async () => {
    ...
    const answersToSave = Object.entries(wellbeingStore.answers).map(([key, value]) => {
...
onMounted(async () => {
	try {
		const response = await getAnswers(farmId, 'animal_welfare');

		if (response.length > 0) {
			response.forEach(answer => {
        		// Check for string_answer
        		if (answer.string_answer !== undefined && answer.string_answer !== '') {
           			wellbeingStore.saveAnswer(answer.question_key, answer.string_answer);
        		}
        		...
        		// Check for string_array_answer (non-empty array)
            		...
            		wellbeingStore.saveAnswer(answer.question_key, answer.string_array_answer);
        		}

       			// Check for numeric_answer
        		else if (answer.numeric_answer !== undefined && !isNaN(answer.numeric_answer)) {
            		wellbeingStore.saveAnswer(answer.question_key, answer.numeric_answer);
        		}
        		...
        		// Check for numeric_array_answer (non-empty array)
            		...
            		wellbeingStore.saveAnswer(answer.question_key, answer.numeric_array_answer);
        		}
    		});
		};
